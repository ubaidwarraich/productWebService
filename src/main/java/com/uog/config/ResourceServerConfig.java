        ...
        resource.setConfigurers(
                Collections.<ResourceServerConfigurer>singletonList(new ResourceServerConfigurerAdapter() {
                    ...
                    public void configure(HttpSecurity http) throws Exception {
                        ...
                        http.requestMatchers().antMatchers("/product", "/product/**").and().authorizeRequests()
                                .anyRequest().access("#oauth2.hasScope('write')");
                        http.requestMatchers().antMatchers("/productcategory", "/productcategory/**").and().authorizeRequests()
                        .anyRequest().access("#oauth2.hasScope('write')");
                    }
                }));
